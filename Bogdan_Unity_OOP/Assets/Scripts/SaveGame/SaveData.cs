...
namespace Game
{
    ...
    public sealed class SaveData
    {
        ...
        public string[] listOfInteractiveObjects;

        public float LengthOfObjects;
        public override string ToString() => $"Name {Name} Position {Position} IsVisible {IsActive} Points {Points}" ;


        public string[] ConvertObjects(InteractiveObject[] interactiveObjects)
        {
            string[] result = new string[] { };

            for (int i = 0; i < interactiveObjects.Length; i++)
            {
                result[i] = interactiveObjects[i].name;
            }
            return result;
        }
    }
